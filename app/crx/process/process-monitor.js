...
showProcessInfo = function(processes){
  document.getElementById("result-area").innerHTML = JSON.stringify(processes);
}
...
function init() {
  ...
  document.getElementById("killProcess").onclick = function () {
    var procId = parseInt(document.getElementById("pid").value);
    chrome.processes.terminate(procId);
  }
  
  document.getElementById("getProcessInfo").onclick = function () {
    var procId = parseInt(document.getElementById("pid").value);
    chrome.processes.getProcessInfo(procId, true, showProcessInfo);
  }
}
